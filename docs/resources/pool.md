...
subcategory: ""
description: |-
  Registers a top-level IP pool — the parent CIDR block that nxip_allocation resources carve non-overlapping subnets from. A pool is scoped to exactly one address family per environment/region: to support both IPv4 and IPv6 for the same environment/region, create two pools (one per family), not one pool with a mixed range.
---
